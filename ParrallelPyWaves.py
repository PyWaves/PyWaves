...
class ParallelPyWaves(object):
    ...
    def PAddress(self, address='', publicKey='', privateKey='', seed='', alias='', nonce=0):
        return Address(self, address, publicKey, privateKey, seed, alias, nonce)

    def PAsset(self, assetId):
        return Asset(assetId, self)

    def PAssetPair(self, asset1, asset2):
        return pyAssetPair(asset1, asset2, self)

    def POrder(self, orderId, assetPair, address = ''):
        return Order(self, orderId, assetPair, address)

    def PContract(self, contractAddress, seed):
        return Contract(contractAddress, seed, self)

    def POracle(self, oracleAddress, seed):
        return Oracle(self, oracleAddress, seed)
